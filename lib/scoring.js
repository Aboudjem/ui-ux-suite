...
function scoreTypography(typeData) {
  ...
  if (typeData.bodySize && typeData.bodySize < 14) {
    score -= 2;
    findings.push({ severity: 'critical', msg: `Body text too small (${typeData.bodySize}px) - minimum 14px, prefer 16px`, laws: ['aesthetic-usability-effect'] });
...
function scoreAccessibility(a11yData) {
  ...
  // Accessibility findings cite the WCAG success criterion, not a UX law (a UX law is not
  // evidence for a WCAG failure — corrected per the UX-law verification audit).
  if (a11yData.contrastFailures > 0) {
    score -= Math.min(3, a11yData.contrastFailures * 0.5);
    findings.push({ severity: 'critical', msg: `${a11yData.contrastFailures} contrast ratio failures`, wcag: ['1.4.3'] });
  }

  if (!a11yData.hasFocusIndicators) {
    score -= 2;
    findings.push({ severity: 'critical', msg: 'No visible focus indicators for keyboard navigation', wcag: ['2.4.7'] });
  }

  if (!a11yData.hasSkipLink) {
    score -= 0.5;
    findings.push({ severity: 'suggestion', msg: 'No skip-to-content link (bypass blocks)', wcag: ['2.4.7'] });
  }

  if (a11yData.missingAltText > 0) {
    score -= Math.min(2, a11yData.missingAltText * 0.3);
    findings.push({ severity: 'important', msg: `${a11yData.missingAltText} images missing alt text`, wcag: ['1.1.1'] });
  }
...
function scoreHierarchy(d) {
  ...
  if (!d.has2xlOrLarger) { score -= 1; findings.push({ severity: 'suggestion', msg: 'No display-size type (text-2xl or larger) — hero text likely underscaled for visual anchoring', laws: ['von-restorff-effect'] }); }
...
function formatScoreCard(scoreCard, options = {}) {
  const { includeTopFindings = true } = options;
  const lines = [];
  ...
  if (includeTopFindings && scoreCard.topFindings && scoreCard.topFindings.length > 0) {
    lines.push('\n## Top Findings\n');
