...
// A file's "site root" = its top-level path segment (root-level files share '__root__'). Contrast
// must be resolved per site, not per pooled project: a monorepo with a light site + a dark site
// would otherwise cross-contaminate var maps / page surfaces and emit impossible findings
// (skeptic VERIFICATION-2 NO-GO: hermes-personal-brand). Single apps (all under src/ or root) stay one group.
function siteOf(file) {
  if (!file) return '__root__';
  const i = file.indexOf('/');
  return i === -1 ? '__root__' : file.slice(0, i);
}

// --- Detector: text contrast + surface separation (delegates to static-contrast), scoped per site ---
function detectContrast(cssDecls, seed) {
  const findings = [];
  let n = seed;
  const groups = new Map();
  for (const d of cssDecls) {
    const k = siteOf(d.file);
  ...
  for (const decls of groups.values()) {
    textSeeds = textSeeds.concat(analyzeTextContrast(decls));
    surfaceSeeds = surfaceSeeds.concat(analyzeSurfaceSeparation(decls));
  }
  const { singles, summaries } = groupSeries(textSeeds);
