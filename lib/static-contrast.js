...
// A container surface need only be MOSTLY opaque (a >= 0.5) to define what its descendants sit on
// (e.g. a cookie banner `rgba(26,26,26,.95)`).
function containerSurfaceHex(value, varMap) {
  if (!value || /gradient|url\(|image/i.test(value)) return null;
  const c = resolveColorRGBA(value, varMap);
  return c && c.a >= 0.5 ? rgbToHexLocal(c.r, c.g, c.b) : null;
}

// BEM-ish block roots of a class/id token: `.social-proof-item` → ['.social-proof-item','.social-proof','.social'].
function blockRoots(token) {
  const m = token.match(/^([.#])([\w]+(?:-[\w]+)*)$/);
  if (!m) return [token];
  const sep = m[1];
  const parts = m[2].split('-');
  const roots = [];
  for (let i = parts.length; i >= 1; i--) roots.push(sep + parts.slice(0, i).join('-'));
  return roots;
}
const classTokens = simple => (simple.match(/[.#][\w-]+/g) || []);
const AMBIG = '__ambiguous__';

// Map of container key (class/id + its block roots) → the surface hex its descendants sit on.
// A key whose registered surfaces straddle light AND dark is marked ambiguous (ignored later).
function buildContainerSurfaceMap(declarations, varMap) {
  const map = {};
  // Interactive controls own their own surface; they are NOT ancestor containers for sibling text.
  // Registering them would poison a block root (e.g. light `.cookie-accept` button vs dark
  // `.cookie-banner` → ambiguous `.cookie`). Exclude them from the container map.
  const CONTROL = /(btn|button|accept|decline|^a$| a$|link|input|select|textarea|icon|chip|pill|tag|badge|avatar|toggle|close|submit|cta|field|switch|checkbox|radio|divider|offer|dot|line|rule|accent|swatch|thumb|bullet|marker|glow|blob|shape|deco|star|arrow)/i;
  // A SHORTENED block root (e.g. `.cookie` from `.cookie-banner`) may only be claimed as a surface
  // when the element is a genuine CONTAINER (its last segment is a container word) — not a decorative
  // child like `.ba-divider`/`.popup-offer`, which would wrongly lend its accent color to sibling text.
  const CONTAINER_SUFFIX = /^(banner|bar|section|footer|header|nav|navbar|hero|overlay|wrapper|panel|drawer|sidebar|menu|sheet|dialog|card|container|region|popup|modal|backdrop|shell|inner|body|main|aside|block|group|grid|content|area|box|well|tile|strip|board|toolbar|topbar|appbar|masthead|callout|billboard|marquee|wrap|stage|frame|surface|page|layout|cookie)$/i;
  const register = (root, surface) => {
    if (map[root] === AMBIG) return;
    if (map[root] && map[root] !== surface) {
      const a = luminanceOf(map[root]) >= 0.5, b = luminanceOf(surface) >= 0.5;
      if (a !== b) { map[root] = AMBIG; return; }
    }
    map[root] = surface;
  };
  for (const d of declarations) {
    if (d.prop !== 'background' && d.prop !== 'background-color') continue;
    const surface = containerSurfaceHex(d.value, varMap);
    if (!surface) continue;
    const simples = (d.selector || '').split(/\s+|>|\+|~/).filter(Boolean);
    const target = simples[simples.length - 1];
    if (!target || CONTROL.test(target)) continue;
    for (const tok of classTokens(target)) {
      const roots = blockRoots(tok);                 // [full, shorter, …, block]
      const lastSeg = (tok.replace(/^[.#]/, '').split('-').pop()) || '';
      const isContainer = CONTAINER_SUFFIX.test(lastSeg) || roots.length === 1;
      register(roots[0], surface);                   // the element's own surface always registers
      if (isContainer) for (let i = 1; i < roots.length; i++) register(roots[i], surface);
    }
  }
  return map;
}

// Infer the real surface a backgroundless element sits on, from descendant-combinator ancestors and
// BEM block-root co-membership (e.g. `.footer-brand-name span` → `.footer` → espresso). Returns a hex,
// or null when nothing matches or candidates conflict.
function inferSurface(selector, containerMap) {
  if (!selector) return null;
  const simples = selector.split(/\s+|>|\+|~/).filter(Boolean);
  const found = new Set();
  for (const simple of simples) {
    for (const tok of classTokens(simple)) {
      for (const root of blockRoots(tok)) {
        const s = containerMap[root];
        if (s && s !== AMBIG) found.add(s);
      }
    }
  }
  if (found.size === 0) return null;
  const arr = [...found];
  const hasLight = arr.some(h => luminanceOf(h) >= 0.5);
  const hasDark = arr.some(h => luminanceOf(h) < 0.5);
  if (hasLight && hasDark) return null; // conflicting candidate surfaces
  return arr[0];
}
...
function analyzeTextContrast(declarations) {
  ...
  const containerMap = buildContainerSurfaceMap(declarations, varMap);
  ...
  for (const { file, selector, decls } of rules.values()) {
    ...
    // Resolve the effective surface. A rule "owns" its surface only if it sets a fully-opaque solid
    // background; a gradient / transparent / translucent / unresolved background falls back to the page
    // surface, which (for a nested element) may not be its real ancestor surface — so mark it bgFromPage.
    const bgDecl = decls.find(d => d.prop === 'background-color' || d.prop === 'background');
    const ownSolid = bgDecl ? solidBackgroundHex(bgDecl.value, varMap) : null;
    let bg, bgFromPage;
    if (ownSolid) { bg = ownSolid; bgFromPage = false; }
    else {
      // No own opaque background: infer the real ancestor section surface (descendant / BEM block
      // root) before falling back to the page — so white text in a dark footer is measured on the
      // footer, not the light page (the round-4 fix). Compose any translucent own bg over that base.
      const inferred = inferSurface(selector, containerMap);
      const base = inferred || pageBg;
      bg = resolveBackgroundHex(bgDecl ? bgDecl.value : null, varMap, base);
      if (!bg) continue; // surface indeterminate (var-themed/dark/translucent over unknown) — skip, never assume
      bgFromPage = !inferred; // a page fallback is only "from page" when no real surface was inferred
    }
    ...
    // The page-fallback surface is an APPROXIMATION of where a backgroundless element actually sits, so
    // emit only an UNAMBIGUOUS failure. Skip (a) near-invisible (<1.5:1) — almost always a missing
    // darker-ancestor surface (white text in a dark section: real ratio ~16:1), and (b) MARGINAL fails
    // within 0.5 of threshold — the element's true surface may be lighter (e.g. a white card on a cream
    // page), flipping a 4.43 "fail" into a 4.73 pass. A finding with its OWN background is always exact
    // and bypasses this band (e.g. PLANTED A #fbfbfb on its own #ffffff still flags critical).
    if (bgFromPage && (ratio < 1.5 || ratio >= threshold - 0.5)) continue;
    if (ratio < threshold) {
