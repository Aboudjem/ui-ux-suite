...
function buildContainerSurfaceMap(declarations, varMap) {
  ...
  const CONTROL = /(btn|button|accept|decline|^a$| a$|link|input|select|textarea|icon|chip|pill|tag|badge|avatar|toggle|close|submit|cta|field|switch|checkbox|radio)/i;
  for (const d of declarations) {
...
function analyzeTextContrast(declarations) {
  ...
  for (const { file, selector, decls } of rules.values()) {
    ...
    // A backgroundless element that resolves to a NEAR-INVISIBLE ratio on the PAGE surface is almost
    // always sitting on a differently-colored ancestor that flat CSS can't see — e.g. white text in a
    // dark footer/section on a light page (real ratio ~16:1, not 1.03:1). Emitting a critical here, with
    // a destructive suggested fix, is the missing-surface artifact behind every prior review round.
    // Skip it. Genuine near-invisible text with its OWN background (e.g. #fbfbfb on #ffffff) still flags.
    if (bgFromPage && ratio < 1.5) continue;
    if (ratio < threshold) {
