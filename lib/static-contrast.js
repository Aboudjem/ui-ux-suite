...
// Build a CSS custom-property map (--name -> raw value). Base (non-@media) definitions WIN over
// at-rule (e.g. @media dark) redeclarations, so a `:root` value isn't clobbered by a dark-mode
// override (the v0.4 fix for `var()` collisions emitting impossible fg===bg findings).
function buildVarMap(declarations) {
  const base = {};
  const atRule = {};
  for (const d of declarations) {
    if (!d.prop || !(d.prop.startsWith('--') || d.prop.startsWith('$'))) continue; // CSS custom props + SCSS vars
    if (d.atRule) { if (!(d.prop in atRule)) atRule[d.prop] = d.value; }
    else base[d.prop] = d.value;
  }
  return Object.assign({}, atRule, base); // base overrides atRule
}
