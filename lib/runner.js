...
const { runLocatedAudit } = require('./located-audit');
const { codeMask } = require('./locator');
...
// Test a regex against content but ignore matches inside comments/string literals.
function maskedTest(content, regex) {
  const mask = codeMask(content);
  const re = new RegExp(regex.source, regex.flags.includes('g') ? regex.flags : regex.flags + 'g');
  let m;
  while ((m = re.exec(content)) !== null) {
    if (m.index === re.lastIndex) re.lastIndex++;
    if (mask[m.index] === 1) return true;
  }
  return false;
}

const SEVERITY_RANK = { critical: 0, important: 1, suggestion: 2, 'nice-to-have': 3 };

// Merge located findings into the scorecard: attach to dimensions (for counts + laws coverage),
// apply a bounded score penalty per dimension, and rebuild a ranked, located-first topFindings list.
function applyLocatedFindings(scoreCard, locatedFindings) {
  const byDim = {};
  for (const f of locatedFindings) {
    (byDim[f.dimension] = byDim[f.dimension] || []).push(f);
    // back-compat: formatScoreCard + laws coverage read `msg`
    f.msg = f.title;
    f.located = true;
  }
  for (const dim of scoreCard.dimensions) {
    const located = byDim[dim.id] || [];
    if (located.length === 0) continue;
    dim.findings = located.concat(dim.findings || []);
    // bounded penalty: real located issues must lower the score (kills the "white-on-white = 5.4" bug)
    const crit = located.filter(f => f.severity === 'critical').length;
    const imp = located.filter(f => f.severity === 'important').length;
    const sug = located.filter(f => f.severity === 'suggestion').length;
    const penalty = Math.min(4, crit * 1.2 + imp * 0.6 + sug * 0.15);
    if (dim.score != null) dim.score = Math.max(1, Math.round((dim.score - penalty) * 10) / 10);
    dim.locatedCount = located.length;
  }
  calculateOverall(scoreCard);
  // Rebuild topFindings: located findings first (within each severity), then aggregate.
...
function auditProject(projectPath, options = {}) {
  ...
  for (let i = 0; i < jsxSampleSize; i++) {
    try {
      const content = fs.readFileSync(jsxFiles[i], 'utf8');
      // Mask-aware: a literal `focus-visible` inside a comment/string must NOT count (v0.3 bug).
      if (maskedTest(content, /:focus-visible|focus-visible:|focus:|outline-/)) a11ySignals.hasFocusVisible = true;
      ...
      if (maskedTest(content, /prefers-reduced-motion/)) a11ySignals.hasReducedMotion = true;
  ...
  for (const f of cssFiles) {
    try {
      const content = fs.readFileSync(f, 'utf8');
      if (maskedTest(content, /:focus-visible|:focus\b/)) a11ySignals.hasFocusVisible = true;
      if (maskedTest(content, /prefers-reduced-motion/)) a11ySignals.hasReducedMotion = true;
    } catch {}
  }
  ...
  const scoreInput = {
    ...
    typography: {
      ...
      bodySize: (() => { const body = sizeNums.filter(n => n >= 8 && n <= 24); return body.length > 0 ? Math.min(...body) : (sizeNums.length > 0 ? Math.min(...sizeNums) : null); })(),
      bodyLineHeight: null,
    },
    layout: {
    ...
    performance: {
      // Next.js-specific advice ("use next/font", "use next/image") only applies to Next/Nuxt
      // projects — neutralize on other stacks so it stops firing false positives (the real
      // image issues are caught by located alt/markup detectors regardless).
      hasNextFont: (profile.framework === 'next' || profile.framework === 'nuxt') ? hasNextFont : true,
      hasNextImage: (profile.framework === 'next') ? hasNextImage : true,
      hasSuspense: (profile.framework === 'next' || profile.framework === 'react') ? hasSuspense : true,
      hasSkeleton,
      imagesInJsx,
  ...
  // --- Located findings: the specificity layer (element/selector/file:line + measured + fix) ---
  let located = { findings: [], stats: {} };
  try { located = runLocatedAudit(projectPath); } catch (e) { located = { findings: [], stats: { error: e.message } }; }
  applyLocatedFindings(scoreCard, located.findings);

  const htmlFiles = walkFiles(projectPath, ['.html', '.htm']);

  // --- Trust guard: refuse to score a directory with no analyzable evidence ---
  const totalEvidence = cssFiles.length + jsxFiles.length + htmlFiles.length;
  const insufficientEvidence = totalEvidence === 0;
  if (insufficientEvidence) {
    scoreCard.overall = null;
    scoreCard.grade = null;
    for (const dim of scoreCard.dimensions) { dim.score = null; dim.findings = []; }
...
function renderLaws(f) {
  const parts = [];
  for (const slug of (f.laws || [])) {
    const meta = LAW_META[slug];
    if (meta) parts.push(`[${meta.name}](${meta.url})`);
  }
  for (const sc of (f.wcag || [])) {
    const label = WCAG_SC[sc] || sc;
    parts.push(`WCAG ${sc} ${label}`);
  }
  return parts.length ? parts.join(' · ') : '';
}

// Rich, located finding block: title + where + measured + fix + before/after + citations.
function renderFinding(f) {
  const icon = f.severity === 'critical' ? '[!]' : f.severity === 'important' ? '[*]' : f.severity === 'suggestion' ? '[-]' : '[ ]';
  const out = [];
  out.push(`### ${icon} ${f.title || f.msg}`);
  const e = f.evidence || {};
  const locBits = [];
  if (e.file) locBits.push(`\`${e.file}${e.line != null && e.line > 0 ? ':' + e.line : ''}\``);
  if (e.selector) locBits.push(`selector \`${e.selector}\``);
  out.push(`- **Where:** ${locBits.length ? locBits.join(' · ') : '_project-level_'}`);
  if (e.measured) out.push(`- **Measured:** ${e.measured}${e.threshold ? ` (needs ${e.threshold})` : ''}`);
  if (f.description) out.push(`- **Why:** ${f.description}`);
  ...
  const cites = renderLaws(f);
  if (cites) out.push(`- **Cites:** ${cites}`);
  if (e.screenshot) out.push(`- **Screenshot:** ![annotated](${e.screenshot})`);
  return out.join('\n');
}
...
function formatReport(audit) {
  ...
  lines.push(`**Files scanned:** ${audit.files.css} CSS, ${audit.files.jsx} JSX/TSX/Vue/Svelte, ${audit.files.html || 0} HTML\n`);

  if (audit.insufficientEvidence) {
    lines.push('> **Insufficient evidence.** No CSS, JSX/TSX/Vue/Svelte, or HTML files were found at this path, ');
    lines.push('> so there is nothing to audit and **no score is reported**. Point the tool at a project root that ');
    lines.push('> contains stylesheets and/or components (or pass a running URL with `depth: "deep"` for a live audit).');
    return lines.join('\n');
  }
  ...
  lines.push(formatScoreCard(audit.scoreCard, { includeTopFindings: false }));
  ...
  lines.push(`- Breakpoints: ${audit.layout.breakpoints.length ? audit.layout.breakpoints.join('px, ') + 'px' : 'none detected'}`);
  lines.push(`- Container max-widths: ${audit.layout.containerWidths.widths.length ? audit.layout.containerWidths.widths.join('px, ') + 'px' : 'none detected'}\n`);

  const allFindings = audit.scoreCard.topFindings || [];
  const located = allFindings.filter(f => f.located);
  const projectLevel = allFindings.filter(f => !f.located);

  if (located.length > 0) {
    const total = (audit.located && audit.located.findings && audit.located.findings.length) || located.length;
    lines.push(`## Specific findings (located + measured + fixed)\n`);
    lines.push(`${total} located finding(s). The highest-impact are detailed below; each names the exact element, the measured value, and the concrete fix.\n`);
    for (const f of located.slice(0, 15)) lines.push(renderFinding(f) + '\n');
    if (located.length > 15) lines.push(`_…and ${total - 15} more located findings (see JSON output for the full list)._\n`);
  }

  if (projectLevel.length > 0) {
    lines.push('## Project-level observations\n');
    lines.push('These are systemic (not tied to one line); evidence is the project as a whole.\n');
    for (const f of projectLevel.slice(0, 12)) {
      const icon = f.severity === 'critical' ? '[!]' : f.severity === 'important' ? '[*]' : '[-]';
      const cites = renderLaws(f);
      lines.push(`- ${icon} **${f.dimension}** — ${f.msg || f.title}${cites ? ` _(${cites})_` : ''}`);
    }
    lines.push('');
  }
