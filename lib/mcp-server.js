...
const TOOLS = [
  ...
  },
  {
    ...
    description: 'Query the design knowledge base. Searches both the structured knowledge object and 21 markdown knowledge documents.',
    inputSchema: {
      type: 'object',
      properties: {
...
async function handleAuditRun({ projectPath, baseUrl, depth = 'quick', dimensions, format = 'full' } = {}) {
  ...
  if (audit.insufficientEvidence) warnings.push('Insufficient evidence: no CSS, JSX/TSX/Vue/Svelte, or HTML files were found — no score is reported. Check the path or pass a running URL with depth:"deep".');
  else if (audit.files.css === 0 && audit.files.jsx === 0) warnings.push('No CSS or JSX/TSX/Vue/Svelte files found — check that the project path is correct or adjust ignored-dirs.');
  ...
  const located = (audit.located && audit.located.findings) || [];
  const locatedFiltered = dimensions && dimensions.length > 0
    ? located.filter(f => dimensions.includes(f.dimension))
    : located;
  ...
  const result = {
    ...
    filesScanned: {
      css: audit.files.css,
      jsx: audit.files.jsx,
      html: audit.files.html || 0,
      total: audit.files.css + audit.files.jsx + (audit.files.html || 0),
    },
    profile: audit.profile,
    overall: sc.overall,
    grade: sc.grade,
    perDimension,
    locatedFindingCount: locatedFiltered.length,
    locatedFindings: locatedFiltered,
    topFindings: filteredTopFindings.slice(0, 20),
  ...
  // Deep-mode weave: promote live axe violations + touch-target sizes (with selectors and
  // annotated screenshots) into located findings, so deep mode enriches — not bypasses — the
  // same finding model as the static path. Runs before the format branches so every format sees it.
  if (browserResult && browserResult.ok) {
    result.browserAudit = browserResult;
    try {
      const { weaveBrowserFindings } = require('./browser-weave');
      const deep = weaveBrowserFindings(browserResult) || [];
      if (deep.length) {
        result.locatedFindings = deep.concat(result.locatedFindings || []);
        result.locatedFindingCount = result.locatedFindings.length;
        result.deepFindingCount = deep.length;
        const SR = { critical: 0, important: 1, suggestion: 2, 'nice-to-have': 3 };
        result.topFindings = result.locatedFindings
          .concat((result.topFindings || []).filter(f => !f.located))
          .sort((a, b) => (SR[a.severity] ?? 4) - (SR[b.severity] ?? 4))
          .slice(0, 25);
      }
    } catch (e) {
      warnings.push('deep-mode finding weave unavailable: ' + e.message);
  ...
  if (format === 'summary') {
    return {
      ...
      locatedFindingCount: result.locatedFindingCount,
