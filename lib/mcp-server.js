...
const { queryKnowledge, searchKnowledge } = require('./knowledge');
...
const TOOLS = [
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        brandColor: { type: 'string', description: 'Primary brand color (hex)' },
        style: { type: 'string', enum: Object.keys(require('./knowledge').KNOWLEDGE.styleDirections), description: 'Visual style direction' },
  ...
  },
  {
    ...
    description: 'Query the built-in design knowledge base for principles, patterns, and recommendations.',
    inputSchema: {
      type: 'object',
      properties: {
        category: { type: 'string', enum: ['color', 'typography', 'components', 'accessibility', 'layout', 'interaction', 'styleDirections'], description: 'Knowledge category' },
        key: { type: 'string', description: 'Dot-separated key path (e.g., "harmony.complementary")' },
        search: { type: 'string', description: 'Free-text search across all knowledge (alternative to category+key)' },
      },
    },
...
async function handleScanProject({ projectPath }) {
  ...
  // Gather file list
  const filePatterns = ['**/*.css', '**/*.scss', '**/*.tsx', '**/*.jsx', '**/*.vue', '**/*.svelte'];
  // In practice, this would use glob — for now, return the detection logic
  profile.framework = detectFramework(packageJson);
  profile.componentLib = detectComponentLib(packageJson);

  // Check for shadcn via components.json
  try {
...
async function handleExtractColors({ projectPath }) {
  ...
  return {
    ...
    colors: allColors.slice(0, 100), // cap for response size
...
async function handleKnowledgeQuery({ category, key, search }) {
  if (search) {
    return { success: true, results: searchKnowledge(search) };
  }
  if (category) {
    const result = queryKnowledge(category, key);
    return { success: true, result };
  }
  return { success: false, error: 'Provide either category+key or search' };
}

async function handleAuditLog(params) {
  const finding = createFinding(params);
  auditLog.push(finding);
  return { success: true, findingId: finding.id, totalFindings: auditLog.length };
}
...
if (require.main === module) {
  ...
  // Standard MCP server setup would go here
  console.log('UI/UX Suite MCP Server — ' + TOOLS.length + ' tools available');
}

module.exports = { TOOLS, findFiles };
